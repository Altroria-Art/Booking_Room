...
const canUpdate = computed(
  () =>
    form.roomId !== "" && !!form.room_code?.trim() && form.room_type_id !== ""
);

// —— แก้ไขห้อง ——
const submitUpdate = async () => {
  if (!canUpdate.value)
    return alert("กรุณาเลือกห้อง และกรอกเลขห้อง/ประเภทให้ครบ");
  ...
  <teleport to="body">
    <div v-if="open" class="backdrop" @click.self="close">
      <div class="sheet">
        <header class="head">
          <h2>
            <span class="hl">แก้ไข/ลบห้อง</span
            ><span class="sub">STUDYROOM</span>
          </h2>
        </header>
        ...
        <section class="body">
          ...
          <label class="label"
            >กรุณาเลือกประเภทห้องที่ต้องการเปลี่ยน <b>*</b></label
          >
...
/* ✅ ทำให้ input/select คำนวณความกว้างเหมือนกัน */
.control {
  width: 100%;
  ...
  background: #fff;
  font: inherit; /* ให้ขนาดฟอนต์เท่ากัน */
  line-height: 1.25;
}

/* ✅ ทำให้ select ดูสม่ำเสมอทุกเบราว์เซอร์ */
select.control {
  appearance: none;
